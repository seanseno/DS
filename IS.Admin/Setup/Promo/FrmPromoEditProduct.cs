...
{
    public partial class FrmPromoEditProduct : BaseForm
    {
        ISFactory factory = new ISFactory();
        IList<TempPromoDetails> _list = new List<TempPromoDetails>();
        int _PromoId { get; set; }
        public FrmPromoEditProduct(int PromoId)
        {
            InitializeComponent();
        ...
        private void loadData()
        {
            ...
            var promo = factory.PromoRepository.GetList().Where(x => x.Id == _PromoId).FirstOrDefault();
            txtPromoName.Text = promo.PromoName;
            chkActive.Checked = promo.Active == 1 ;
        }
        ...
        private bool CheckInput()
        {
            ...
            else if (factory.PromoRepository.GetList().Where(x=>x.PromoName.ToUpper()==txtPromoName.Text.ToUpper() && x.Id != _PromoId).Count() > 0)
            {
        ...
        private void FrmPromoEditProduct_Load(object sender, EventArgs e)
        {
            var obj = factory.PromoDetailsRepository.GetList().Where(x=>x.PromoId==_PromoId).OrderBy(y => y.ProductId).ToList();
            foreach (var itm in obj)
            {
                var temp = new TempPromoDetails();
                temp.ProductId = itm.ProductId;
